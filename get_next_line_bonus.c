...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/20 12:47:52 by ouidriss          #+#    #+#             */
/*   Updated: 2022/12/20 13:10:16 by ouidriss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	buffer = set_full_buffer_size(fd, remaining[fd]);
	if (!buffer)
		return (remaining[fd] = NULL, NULL);
	remaining[fd] = set_remaing_result(buffer);
